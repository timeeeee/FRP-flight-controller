...
ekf xs ys zs = let ((beginX, endX),(beginZ, endZ), x_e) = ((splitAt 9 xs),(splitAt 9 zs), (x_est_ endZ ys))
								in (add_list (mult_list (subElem (y_ beginX) (y_est_ x_e)) (k_function endZ ys)) x_e)
						
add_list :: [Float] -> [Float] -> [Float]
add_list xs ys = (map add1 (zip xs ys))

add1 :: (Float, Float) -> Float
add1 x = (fst x) + (snd x)

mult_list :: [Float] -> [Float] -> [Float]
mult_list xs ys = xs 
